...
- **A Windows desktop pairs relay-only — no SSH key, and do not "fix" that by writing one.** The
  ...
  would then refuse every admin key in it). Relay attach on Windows needs the session host
  shipped (#575/#579) — without it `pty.attach` spawns a new plain shell, not the node's session.
